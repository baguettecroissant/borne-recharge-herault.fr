#!/usr/bin/env node
/**
 * Enriches communes.json with calculated SEO-unique fields for Hérault (34):
 * - distanceMontpellier (distance to Montpellier center)
...
function getProfilCommune(pop) {
  if (pop >= 250000) return 'métropole majeure régionale';
  if (pop >= 50000) return 'ville majeure d\'agglomération';
  if (pop >= 15000) return 'ville moyenne dynamique';
  if (pop >= 8000) return 'commune périurbaine résidentielle';
  if (pop >= 4000) return 'bourg languedocien';
  return 'commune ou village résidentiel';
}

function getMarcheImmobilier(prixM2) {
  if (!prixM2) return 'non renseigné';
  if (prixM2 >= 4000) return 'premium (littoral / banlieue chic)';
  if (prixM2 >= 3200) return 'très dynamique (Montpellier & Sète)';
...
function getTauxMaisonLabel(pct) {
  ...
  if (pct >= 80) return 'très majoritairement composé de villas individuelles';
  if (pct >= 60) return 'principalement pavillonnaire et résidentiel';
  if (pct >= 40) return 'mixte habitat individuel et collectif';
  if (pct >= 20) return 'majoritairement collectif de standing';
  return 'très dense et collectif (coeur de métropole)';
}

let enriched = 0;
for (const c of communes) {
  // Distance to Montpellier
  if (c.latitude && c.longitude) {
    c.distanceMontpellier = Math.round(haversineKm(c.latitude, c.longitude, MONTPELLIER_LAT, MONTPELLIER_LON) * 10) / 10;
  } else {
    c.distanceMontpellier = null;
  }
...
console.log(`✅ Enriched ${enriched} communes with Hérault (34) SEO data fields.`);

// Print sample
const sample = communes[0];
console.log('\nSample (Montpellier):');
console.log(`  distanceMontpellier: ${sample.distanceMontpellier} km`);
